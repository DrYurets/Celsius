...
- **Sunrise/sunset** (Open-Meteo `daily=sunrise,sunset`, уже в URL): парсятся в RTC (`weatherSunriseHour/Min`, `weatherSunsetHour/Min`, `weatherSunLocalDate`).
  - Ночной сон **вкл.** (`nightStart` ≠ `nightEnd`): форс сетевого цикла при первом дневном пробуждении, если на сегодня ещё нет данных (не чаще 5 мин при ошибке).
  - Ночной сон **выкл.** (`nightStart` == `nightEnd`, `isNight` всегда false): форс в час `00:xx`.
  - Любой обычный weather fetch тоже обновляет sunrise/sunset.
...
- Ниже: 3 колонки hourly (`час+1..+3`): **час → иконка → T → PoP** (блок сдвинут на **−4 px**, `colY0≈62`); после числа часа — **две метки °** (визуально `HH°°` ≈ прогноз на `HH:00`); **PoP в колонке скрывается при 0%**.
- Низ экрана: точки пути солнца — **1 px на каждый полный час** светового дня **+ 2 px** (восход/закат) по **дуге** (sin): края у низа экрана, вершина на высоте центра иконки; солнце едет по той же дуге; точки **внутри** bbox иконки (~9×9) не рисуются.
